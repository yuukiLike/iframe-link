...
# 只运行 SDK 通信测试
pnpm exec playwright test --config config/playwright.config.ts sdk-communication

# 指定浏览器
pnpm exec playwright test --config config/playwright.config.ts --project=chromium

# 有头模式（看到浏览器界面）
pnpm exec playwright test --config config/playwright.config.ts --headed

# UI 模式（可视化调试，强烈推荐）
pnpm exec playwright test --config config/playwright.config.ts --ui
```
...
- 自动启动本地服务器 (`pnpm exec serve . -l 3456 --cors`)
- 测试目录：`tests/e2e/`
- 当前启用 Chromium
